...
public class HTTPMessageBuilder {
    ...
    public Response getResponse() {
        if (response == null) {
            String state = head.substring(0, head.indexOf("\r\n"));
            ...
            if (first.length >= 3) { 
	            try
	            {
	              responseCode = Integer.parseInt(first[1]);
	              msg = first[2];
	            }
            ...
            response.setResponseCode(responseCode, msg);
            
            //add headers (already set by buildHeaders())  
